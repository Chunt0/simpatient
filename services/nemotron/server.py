...
def direct_transcribe(audio: np.ndarray) -> str:
    ...
    with torch.no_grad():
        processed, processed_len = asr_model.preprocessor(input_signal=audio_tensor, length=audio_len)
        encoded, encoded_len = asr_model.encoder(audio_signal=processed, length=processed_len)
        hypotheses = asr_model.decoding.rnnt_decoder_predictions_tensor(encoded, encoded_len, return_hypotheses=False)
...
def streaming_transcribe(audio: np.ndarray):
    ...
    with torch.no_grad():
        processed, _ = model.preprocessor(input_signal=audio_tensor, length=audio_len)
        ...
        pre_cache_frames = pre_encode_cache[0] if isinstance(pre_encode_cache, (list, tuple)) else pre_encode_cache
        ...
        if pre_cache_frames > 0:
            pad = torch.zeros(processed.shape[0], processed.shape[1], pre_cache_frames, device=device, dtype=processed.dtype)
        ...
        while offset < total_frames:
            ...
            result = model.conformer_stream_step(
                ...
                previous_hypotheses=previous_hypotheses, return_transcription=True,
            )
            _, all_hypotheses, cache_last_channel, cache_last_time, cache_last_channel_len, best_hypothesis = result
            if best_hypothesis and len(best_hypothesis) > 0:
                hypothesis = best_hypothesis[0]
                current_text = hypothesis.text if hasattr(hypothesis, "text") else str(hypothesis)
            elif isinstance(all_hypotheses, list) and len(all_hypotheses) > 0:
                first = all_hypotheses[0]
                current_text = first if isinstance(first, str) else getattr(first, "text", str(first))
            else:
                current_text = ""
...
async def sse_generator(audio: np.ndarray):
    ...
    yield f"data: {json.dumps({'type': 'transcript.text.done', 'text': full_text.strip()})}\n\n"
    yield "data: [DONE]\n\n"
...
async def transcribe(
    ...
    if is_stream:
        return StreamingResponse(sse_generator(audio), media_type="text/event-stream",
    ...
    if response_format == "verbose_json":
        return JSONResponse(content={"text": text, "task": "transcribe", "language": "en", "duration": None})
    return JSONResponse(content={"text": text})


@app.get("/v1/models")
async def list_models():
    return JSONResponse(content={"object": "list", "data": [{"id": MODEL_ID, "object": "model", "created": int(time.time()), "owned_by": "nvidia"}]})
